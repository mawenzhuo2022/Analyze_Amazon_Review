...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    if hasattr(best_model, 'coef_'):
        ...
        # 将特征重要性保存到CSV文件
        feature_importance.to_csv('app/backend/dat/analyze/regression_results/regression_results.csv', index=False)

def main():
    # Load keywords and filtered data, then train and evaluate the model
    # 加载关键词和过滤后的数据，然后训练和评估模型
    keywords = load_keywords('app/backend/dat/analyze/feature_keywords/feature_keywords.csv')
    X_train, y_train = load_and_filter_data('app/backend/dat/analyze/cleaned_data/X_train.csv',
                                            'app/backend/dat/analyze/cleaned_data/Y_train.csv', keywords)
    X_test, y_test = load_and_filter_data('app/backend/dat/analyze/cleaned_data/X_test.csv',
                                          'app/backend/dat/analyze/cleaned_data/Y_test.csv', keywords)
